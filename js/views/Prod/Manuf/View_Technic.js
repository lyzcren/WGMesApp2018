(function($, owner) {
	...
	owner.Buttons = [{
		title: "保存",
		callback: "Manuf_View_Technic.onSaveEventHandler"
	}];

	owner.Self = null;
	owner.Opener = null;
	// 保存列配置名称
	owner.SavedColumnKey = 'App_Manuf_Technic';

	var mCurrentData, mNewData, mCurrentDeptID;
	...
	owner.plusready = function() {
		...
		owner.HeadColumns = Manuf_ConfigColumns.TechnicColumns;
		mMode = owner.Self.Mode;
		mCurrentData = owner.Self.data;
		mCurrentDeptID = owner.Self.FDeptID;
	...
	owner.initElementEvent = function() {
		document.querySelector('[name="FTechnicName"]').addEventListener('tap', function(event) {
			var fQueryString = {
				FDeptID: mCurrentDeptID
			};
			BasicData.pickHandler(event, 'FTechnic', 'FName', 'value', 'FTechnicID', fQueryString);
		});
		document.querySelector('[name="FTechnicName"]').addEventListener('selectedChanged', function(event) {
			var newData = event.detail.newData;
			console
			if(!mNewData) mNewData = {};
			mNewData.FTechnicID = newData.FTechnicID;
		});
		document.querySelector('[name="FValue"]').addEventListener('tap', function(event) {
			var newData = event.detail.newData;
			var fQueryString = {
				FTechnicID: mNewData ? mNewData.FTechnicID : 0
			};
			BasicData.pickHandler(event, 'FTechnicValue', 'FName', 'value', 'FValue', fQueryString);
		});
	};
	...
	owner.refreshHeadData = function(data) {
		mNewData = data;
		owner.maskButtons();
		FormUtil.SetData('#main', mNewData);
	};
	...
	owner.onDeleteEventHandler = function(event) {
		var fTechnicID = document.querySelector('[name="FTechnicID"]').value;
		mui.fire(owner.Opener, 'deleteTechnic', {
	...
	owner.save = function(data) {
		mui.fire(owner.Opener, 'changeTechnic', {
			data: data
		});
		mui.back();
	};
...
}(mui, window.Manuf_View_Technic = {}));

var oTarget = Manuf_View_Technic;